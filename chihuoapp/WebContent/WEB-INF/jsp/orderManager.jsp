...
<body>
	...
	<div class="container">
		...
		<div class="modal hide fade" id="order-modal">
			...
			<div class="control-group">
				<label class="span1"></label> <input type="button" id="over-btn"
					value="结  账" class="btn btn-success span3" onclick="overclick();">
				<input type="button" id="cancel-btn" value="撤  单"
					class="btn btn-success span3" onclick="cancelclick();">
			</div>
		</div>
	</div>
	...
	<script>
		...
		/* 获取所有订单 */
		function getOrders() {
			$.getJSON(RESTURL + "restaurants/" + restaurant.id + "/orders",
					function(data) {
														...
														if (!isNaN(index)) {
															...
															if (value.status != 0) {
																html += "<input type='checkbox' value='"
																		+ value.id
																		+ "' checked disabled  onclick='statusclick("
																		+ value.id
																		+ ")'/>";
															} else {
																html += "<input type='checkbox' value='"
		...
		function statusclick(id) {
			$.ajax({
				type : "PUT",
				url : RESTURL + "restaurants/" + restaurant.id + "/orders/"
						+ selectId + "/" + id,
				cache : false,
