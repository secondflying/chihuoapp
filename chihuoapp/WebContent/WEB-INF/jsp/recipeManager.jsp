...
<body>
		...
		<div class="row">
			<div class="span2">
				<ul class="nav nav-pills nav-stacked">
					...
					<li class="active"><a href="recipeManager">菜品维护</a></li>
	...
	<script>
												...
												function(index, value) {
													if (!isNaN(index)) {
														...
														if (index == 0) {
															restaurant = value;
															document
																	.getElementById("restaurantName").innerText = value.name;
															getCategories();
															return false;
														}
		...
		function getCategories() {
			$
					.getJSON(
							RESTURL + "restaurants/" + restaurant.id
									+ "/categories",
							function(data) {
								$('#cateList').empty();
								$
										.each(
												...
												function(index, value) {
													if (!isNaN(index)) {
														...
														if (selectCid == null) {
															if (index == 0) {
																html = '<li id="cate'+value.id+'" class="active"><a href="#" onclick="cateClick('
																		+ value.id
																		+ ')">'
																		+ value.name
																		+ "</a></li>";
																...
																html = '<li id="cate'+value.id+'"><a href="#" onclick="cateClick('
																		+ value.id
																		+ ')">'
																		+ value.name
																		+ "</a></li>";
										...
										.each(
												...
												function(index, value) {
													if (!isNaN(index)) {
														...
														var html = '<tr><td>'
																+ value.name
																+ '</td><td><img style="width:100px; height:80px;" src="' + IMAGEURL + value.image + '">'
																+ '</td><td>'
																+ value.price
																+ '</td><td></td><td></td><td><a href="#" onclick="editRecipe('
																+ value.id
																+ ')">编辑</a><br /><a href="#" onclick="deleteRecipe('
																+ value.id
																+ ')">删除</a></td></tr>'
														$('#recipeList tbody')
																.append(html);
		...
		function deleteCategory() {
			...
			bootbox.confirm("确定要删除当前选中的菜类吗？", "取消", "确定", function(isOk) {
				if (!isOk) {
					return;
				}
				$.ajax({
					type : "DELETE",
					url : RESTURL + "restaurants/" + restaurant.id
							+ "/categories/" + selectCid,
					cache : false,
		...
		function addRecipe() {
			...
			var rid = $("#rid").val();
			if (rid != null && rid != "") {
				options.url = RESTURL + "restaurants/" + restaurant.id
						+ "/recipes/" + rid;
			}
		...
		function editRecipe(id) {
			$.getJSON(RESTURL + "restaurants/" + restaurant.id + "/recipes/"
					+ id, function(data) {
				$("#rdescription")
